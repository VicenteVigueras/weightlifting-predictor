...
def predict_progress(age, weight, height, experience, goal, exercise, weight_lifted, reps, sets):
    ...
    scaled_features = scaler.transform(input_data[['Age', 'Weight', 'Height', 'WeightLifted', 'Reps', 'Sets']])
    input_data[['Age', 'Weight', 'Height', 'WeightLifted', 'Reps', 'Sets']] = scaled_features
    input_data = np.c_[np.ones(input_data.shape[0]), input_data[['Age', 'Weight', 'Height', 'Experience', 'Goal', 'Exercise', 'WeightLifted', 'Reps', 'Sets']]]
    
    predicted_progress = input_data.dot(beta)

    return predicted_progress[0]

progress = predict_progress(23,75,175,"Beginner","Hypertrophy","Bench Press",45,10,4)
